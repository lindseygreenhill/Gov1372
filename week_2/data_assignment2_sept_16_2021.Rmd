...
win_plot <- tb_wins %>%
  ggplot(aes(x = reorder(winner, n),
             y = n)) +
  geom_col(fill = "azure3") +
  theme_clean() +
  labs(title = "Axelrod Tournament Simulation Number of Wins",
       subtitle = "Backstabber and Handshaker win the most",
       x = "Player",
       y = "Number of Win")
...
tb_points <- player_data_long %>%
  group_by(player) %>%
  summarise(total = sum(score)) %>%
  arrange(desc(total))

points_plot <-  tb_points %>%
  arrange(total) %>%
  ggplot(aes(x = reorder(player, total),
             y = total)) +
  geom_col(fill = "azure3") +
  theme_clean() +
  labs(title = "Axelrod Tournament Simulation Total Points",
       subtitle = "Punisher had most points",
       x = "Player",
       y = "Total Points")

ggarrange(win_plot, points_plot,
          nrow = 2)

# ggsave("blog_2_arranged_bar.png")
```
...
# creating average points for and  against so I can use them in ggplot
...
# making plot using facet wrap

facet_plot <- player_data_long %>%
  ...
  labs(title = "Axelrod Tournament Simulations",
       subtitle =  "Performance of 6 strategies",
       x = "Opponent",
       y = "Score",
       col = NULL) +
  geom_hline(data = avg_points, aes(yintercept = avg_score,
                                    col = "Average Points For")) +
  geom_hline(data = avg_points_against, aes(yintercept = avg_score,
                                            col = "Average Points Against")) +
  scale_color_manual(values = c("indianred", "navyblue"))

facet_plot

# ggsave("blog_2_bar_plot.png")
