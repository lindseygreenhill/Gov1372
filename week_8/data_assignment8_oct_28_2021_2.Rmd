...
```{r}
# I am going to use ID_diff_avg which is the differnce betweence ChinaID_avg and
# JapanID_avg. ChinaDI_avg iis the average of the same 30 identity ratings of
# Chinese people. JapanID_avg is the average of 30 ratings of Japanese people on
# identity trait pairs, coded from 1 to 7 where higher numbers are less
# favorable my contro lvariables are going to be jpfriend (indicator variable
# for if subject has japanese friend or does not), age, and PrejControl
# (motivation to control Prejudice index; an average of 17 items from 1 to 7 in
# which higher scores denote a greater motivation to control the expression of
# prejudice). Okay so in simple terms a positive ID_diff_avg means tha the
# person rated Chinese people higher than Japanese people (which means that they
# view Chinese people less favorably than Japanese people.) A negative score
# indicates that the respondent rated Japanese people higher than Chinese people
# which means that they view Japanese people less favorably than Chinese people.
# I predict that having a japanese friend will have a positive effect on the
# ID_diff_score, meaning that it will go up because the person is more favorable
# towards Japaense people. I think that the age variable will have a negaative
# effect on the DV because older people will be less favorable towards japanese
# people. I think that the PrejControl will have a positive relationship because
# people more motivated to control prejudice will have a more favorable view
...
```{r results="asis"}
ChinaJapan_1 <- ChinaJapan %>%
  mutate(treated = if_else(treated == TRUE, 1, 0))

mod_1 <- lm(ID_diff_avg ~ jpfriend + age + PrejControl + treated, data = ChinaJapan_1)
