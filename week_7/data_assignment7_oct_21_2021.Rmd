...
protest_df_bydate <- protest_df %>% 
  mutate(after = ifelse(day<as.Date("2020-05-25"), 0,1))
```
...
```{r}
# important - 1 = favorable for the police favorability
...
```{r results = "asis"}
# we would want to add race_ethnicity
protest_df_bydate <- protest_df_bydate %>%
  mutate(black = if_else(race_ethnicity == 2, 1, 0),
         over_50 = if_else(age > 50, 1, 0))

# we added an interaction term because we think there could be an additional effect for old republicans

mod_2 <- lm(group_favorability_the_police ~ pid7 + after + black +
               female + over_50*pid7, data = protest_df_bydate)
summary(mod_2)
...
# need to convert to a factor for the model to work 

mod_3 <- polr(data = protest_df_bydate , formula = as_factor(group_favorability_the_police) ~ pid7 + after + black +
               female + over_50*pid7, method = "probit")

summary(mod_3)
# the coefficients are in fact different but they all have the same direction. The interaction doesn't really matter. 
```
...
sec_df <- protest_df_bydate %>%
  drop_na() %>%
  group_by(day_running) %>%
  summarise(avg_score = mean(group_favorability_the_police))

library(ggthemes)

sec_df %>%
  ggplot(aes(y = avg_score, x = day_running)) +
  geom_jitter() +
  geom_vline(xintercept = 0, color = "steelblue2") +
  geom_hline(yintercept = 1.888, color = "indianred") +
  geom_abline(intercept = 2.136, slope = -.00218, color = "navy") +
  theme_clean()

data_after <- sec_df %>%
  filter(day_running > 0)

lm(avg_score ~ day_running, data = data_after)

data_before <- sec_df %>%
  filter(day_running < 0) %>%
  summarise(avg = mean(avg_score))
```
