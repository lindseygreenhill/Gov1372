...
```{r results="asis"}
mod_1 <- lm(init ~ suffrage + polity + polity*suffrage,
            data = s_data)
mod_2 <- lm(init ~ suffrage + polity,
            data = s_data)
...
# filtering for polity score

new_data <- s_data %>%
  filter(polity >= 1) %>%
  mutate(pred_val = predict(mod_1, newdata = .)) 

# creating ggplot graph 
interaction_graph <- new_data %>%
  ggplot(aes(x = polity, y = pred_val)) +
  geom_line(color = "steelblue2") +
  facet_wrap(~suffrage) +
  theme_clean() +
  labs(title = "Regression Predictions of Suffrage vs. No Suffrage",
       subtitle = "0 = No Women's Suffrage, 1 = Women's Suffrage",
       x = "Polity Score",
       y = "Predicted Conflicts")

interaction_graph


# ggsave(plot = interaction_graph, filename = "week_6_plot.png")
