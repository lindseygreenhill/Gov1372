...
```{r}
graph_1 <- marriage_data %>%
  filter(is.na(outPartyUnhappy)==FALSE) %>%
  mutate(strongPARTISAN = if_else(strongPARTISAN == TRUE, "Strong", "Weak"),
         outPartyUnhappy = if_else(outPartyUnhappy == TRUE, "Unhappy", "Not Unhappy")) %>%
  ggplot(aes(x = outPartyUnhappy, y = ..prop.., group=1), stat = "count") +
  geom_bar(fill = "azure3") +
  facet_wrap(~strongPARTISAN) +
  labs(y = "Proportion",
       x =  "Unhappy with Out-Party Marriage",
       title = "Happiness with Out-Party Marriage by Strong Partisanship",
       subtitle = "A larger proportion of strong partisans are unhappy with out-party marriages") +
  theme_clean()

# ggsave("week3_barplot_1.png")

graph_2 <- marriage_data %>%
  filter(is.na(inPartyHappy)==FALSE) %>%
  mutate(strongPARTISAN = if_else(strongPARTISAN == TRUE, "Strong", "Weak"),
         inPartyHappy = if_else(inPartyHappy == TRUE, "Happy", "Unhappy")) %>%
  ggplot(aes(x = inPartyHappy, y = ..prop.., group=1), stat = "count") +
  geom_bar(fill = "azure3") +
  facet_wrap(~strongPARTISAN) +
  labs(y = "Proportion",
       x =  "Happy with In-Party Marriage",
       title = "Happiness with In-Party Marriage by Strong Partisanship",
       subtitle = "Difference not as drastic as Unhappy frame") +
  theme_clean()

# ggsave("week3_barplot_2.png")
```
